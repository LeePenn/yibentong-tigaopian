...
void init() {
            ...
            for (int a = 0; a < 7; a++) {
                for (int b = 0; b < 7; b++) {
                    for (int k = 0; k <= 9; k++) {
                        ...
                        v1.s2 = mod(v1.s2 + j * j % P * (power % P) % P * (power % P) % P * v2.s0 + v2.s2 + 2 * j % P *
                                    (power % P) % P * v2.s1 % P, P);
                    }
                }
            }
    ...
    for (int i = 1; i < N; i++) {
        power7[i] = mod(power7[i - 1] * 10ll, 7);
        power9[i] = mod(power9[i - 1] * 10ll, P);
    }
}
...
int dp(LL n) {
    ...
    for (int i = nums.size() - 1; i >= 0; i--) {
        ...
        for (int j = 0; j < x; j++) {
            ...
            res = mod(res + (last_a % P) * power9[i + 1] % P * (last_a % P) % P * power9[i + 1] % P * v.s0 % P + 2 *
                      (last_a % P) % P * power9[i + 1] % P * v.s1 % P + v.s2, P);
        }
